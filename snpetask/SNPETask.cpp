...
float* SNPETask::getOutputTensor(const std::string& name)
{
    if (isInit()) {
        if (m_outputTensors.find(name) != m_outputTensors.end()) {
            // LOG_INFO("set the buffer encoding type---/////-------: {}.",name);
           std::vector<uint8_t>* out = new std::vector<uint8_t>(m_outputTensors.at(name).data(), m_outputTensors.at(name).data() + 2);
// for (const auto& element : *out) {
//     float* value = reinterpret_cast<float*>(const_cast<unsigned char*>(&element));
//     LOG_INFO("set the buffer encoding type---/////-------: {}.", *value);
// }
